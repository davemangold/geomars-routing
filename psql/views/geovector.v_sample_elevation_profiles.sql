-- View: geovector.v_sample_elevation_profiles

-- DROP VIEW geovector.v_sample_elevation_profiles;

CREATE OR REPLACE VIEW geovector.v_sample_elevation_profiles
 AS
 SELECT this_sample.edge_id,
   ...
   FROM geovector.v_sample_elevation_data this_sample
     JOIN geovector.v_sample_elevation_data next_sample ON this_sample.edge_id = next_sample.edge_id AND (this_sample.point_id + 1) = next_sample.point_id
  ORDER BY this_sample.point_id;

ALTER TABLE geovector.v_sample_elevation_profiles
    OWNER TO postgres;
